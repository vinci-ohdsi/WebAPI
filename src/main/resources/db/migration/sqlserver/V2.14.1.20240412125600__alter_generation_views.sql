...
CREATE VIEW ${ohdsiSchema}.cc_generation as (
  ...
  FROM ${ohdsiSchema}.batch_job_execution job
    JOIN ${ohdsiSchema}.batch_job_execution_params cc_id_param
      ON job.job_execution_id = cc_id_param.job_execution_id
      AND cc_id_param.key_name = 'cohort_characterization_id'
    JOIN ${ohdsiSchema}.batch_job_execution_params source_param
      ON job.job_execution_id = source_param.job_execution_id
      AND source_param.key_name = 'source_id'
    JOIN ${ohdsiSchema}.source s
      ON s.source_id = TRY_CAST(source_param.string_val AS INTEGER)
...
CREATE VIEW ${ohdsiSchema}.estimation_analysis_generation as
  ...
  FROM ${ohdsiSchema}.batch_job_execution job
    JOIN ${ohdsiSchema}.batch_job_execution_params estimation_id_param
      ON job.job_execution_id = estimation_id_param.job_execution_id
      AND estimation_id_param.key_name = 'estimation_analysis_id'
    JOIN ${ohdsiSchema}.batch_job_execution_params source_param
      ON job.job_execution_id = source_param.job_execution_id
      AND source_param.key_name = 'source_id'
    JOIN ${ohdsiSchema}.batch_job_execution_params passwd_param
      ON job.job_execution_id = passwd_param.job_execution_id
      AND passwd_param.key_name = 'update_password'
    JOIN ${ohdsiSchema}.source s
      ON s.source_id = TRY_CAST(source_param.string_val AS INTEGER)
    LEFT JOIN ${ohdsiSchema}.ee_analysis_status exec_info
      ON job.job_execution_id = exec_info.job_execution_id
    LEFT JOIN ${ohdsiSchema}.analysis_generation_info gen_info
      ON job.job_execution_id = gen_info.job_execution_id;
GO
  ...
  (SELECT
   ...
   FROM ${ohdsiSchema}.batch_job_execution job
     JOIN ${ohdsiSchema}.batch_job_execution_params pa_id_param
       ON job.job_execution_id = pa_id_param.job_execution_id
...
CREATE VIEW ${ohdsiSchema}.prediction_analysis_generation as
  SELECT
    ...
    exec_info.id                             analysis_execution_id
  FROM ${ohdsiSchema}.batch_job_execution job
    JOIN ${ohdsiSchema}.batch_job_execution_params plp_id_param
      ON job.job_execution_id = plp_id_param.job_execution_id
